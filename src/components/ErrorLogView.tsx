...
export default function ErrorLogView({
  ...
  const getPoemTitle = (id: string) =>
    poems.find(p => p.id === id)?.title || 'قصيدة';
  ...
  type LogEvent =
    | ({ kind: 'error' } & ErrorLogEntry)
    | ({ kind: 'success' } & SuccessLogEntry);
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        className="w-full max-w-lg !h-[60vh] !max-h-[60vh] !min-h-[60vh] glass-modal rounded-xl overflow-hidden animate-scale-in flex flex-col"
      ...
      >
        ...
        {/* Filters */}
        <div className="px-5 py-3 border-b border-[var(--border-0)] space-y-2 shrink-0">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 py-4">
          <div className="flex flex-col gap-[3px] pb-[200px]">
            {allEvents.length === 0 ? (
              <div className="text-center py-20">
                ...
                <p className="text-base font-bold text-[var(--text-1)]">
                  لا توجد سجلات حاليًا
                </p>

                <p className="text-sm text-[var(--text-3)] mt-1 px-10">
                  ستظهر الأخطاء والنجاحات هنا أثناء مراجعة القصائد
                </p>
              </div>
            ) : (
              allEvents.map(event => (
                    ...
                    <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                      <div className="flex items-center justify-between gap-2 overflow-hidden">
                        <span className="font-bold text-[var(--text-0)] text-sm truncate">
                          {getPoemTitle(event.poemId)} • البيت{' '}
                          {event.verseIndex + 1}
                        </span>

                        <span className="text-[10px] text-[var(--text-3)] font-bold opacity-60 tabular-nums shrink-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        {event.kind === 'error' ? (
                          <Badge variant="error">
                            {getErrorTypeLabel(
                              (event as ErrorLogEntry).errorType
                            )}
                          </Badge>
                        ) : (
        ...
        {/* ملاحظة */}
        <div className="px-5 py-3 border-t border-[var(--border-0)] shrink-0">
