...
export default function Dashboard({ state, onStudy, onNavigate, todaySection }: Props) {
  ...
  return (
   <div className="flex flex-col gap-3 lg:gap-5 pb-[200px] pt-2 lg:pt-0">
      {/* Greeting */}
  ...
  <div className="flex items-center gap-5 w-full">
    ...
    <div className="flex-1 min-w-0 flex flex-col gap-0">
      <div className="flex items-center gap-2">
        <h3 className="text-lg font-bold text-[var(--text-0)] leading-none">تحدي فطين</h3>
          ...
          <Card hover className="p-5 flex flex-col gap-4" onClick={() => onStudy(lastStudied)}>
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0 flex flex-col gap-1">
                <p className="font-bold text-[var(--text-0)] text-base truncate leading-tight relative !right-[10px] top-[10px]">{lastStudied.title}</p>
<p className="text-sm text-[var(--text-2)] truncate leading-tight relative !right-[10px] top-[3px]">{lastStudied.poet}</p>
              </div>
              <Button variant="primary" size="sm" onClick={(e: React.MouseEvent) => { e.stopPropagation(); onStudy(lastStudied); }} className="shrink-0 rounded-full w-10 h-10 p-0 min-h-0 relative !right-[-10px] top-[8px]">
            ...
            <div className="flex flex-col gap-2">
              ...
              <p className="text-[12px] text-[var(--text-1)] font-medium tabular-nums text-left relative !right-[-14px] top-[-4px]">
                ...
                <Card hover key={poem.id} className="p-4" onClick={() => onStudy(poem)}>
                  <div className="flex items-center gap-4">
                    <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                      <p className="font-bold text-[var(--text-0)] text-sm truncate leading-tight relative !right-[10px] top-[4px]">{poem.title}</p>
                      <p className="text-xs text-[var(--text-3)] truncate leading-tight relative !right-[10px] top[4px]">{poem.poet}</p>
                    </div>
                    <div className="flex flex-col items-end gap-1.5 shrink-0">
                      <div className="flex items-center gap-2">
                        {progress && progress.mastered > 0 && <Badge variant="success">متقن</Badge>}
                        <span className="text-xs font-bold text-[var(--text-1)] tabular-nums relative !right-[-10px]">{learnedPct}%</span>
                      </div>
