...
export default function PoemsPage({ state, onAdd, onStudy, onRead, onStats, onDelete }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6 lg:gap-10 pb-[200px]">
      <div className="flex items-center justify-between px-1 gap-4">
        <div className="min-w-0">
          <h1 className="text-2xl font-bold text-[var(--text-0)] leading-tight">
            قصائدي
          </h1>
      ...
      {poems.length === 0 ? (
        <div className="text-center py-20">
          <p className="text-sm text-[var(--text-1)] mb-1">
            لم تضيفي أي قصيدة بعد
          </p>

          <p className="text-xs text-[var(--text-3)] mb-6">
            أضيفي قصيدة وابدئي حفظها بالتكرار المتباعد
          </p>

          <Button variant="primary" onClick={onAdd} className="!px-[30px]">
            إضافة قصيدة
          </Button>
        </div>
      ) : (
        <div className="flex flex-col gap-[8px]">
          {poems.map(poem => {
            ...
            const learnedPct = progress
              ? Math.round(
                  ((progress.mastered + progress.strong) / poem.verses.length) * 100
                )
              : 0;

            const currentVerse = Math.min(
              poem.currentVerseIndex + 1,
              poem.verses.length
            );

            const isDue =
              pStats &&
              Object.values(pStats.verses).some(
                v => Date.now() >= v.nextReviewAt
              );
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  {/* Title + poet */}
                  <div className="flex-1 min-w-0 flex flex-col gap-1 translate-x-[-12px]">

                    <div className="flex items-center gap-2 flex-wrap">
                      <h3 className="text-lg font-bold text-[var(--text-0)] truncate leading-none translate-y-[4px]">
                        {poem.title}
                      </h3>
                      ...
                      {isDue && (
                        ...
                        >
                          <Badge variant="warning">للمراجعة</Badge>
                        </span>
                      )}
                    </div>

                    <p className="text-sm text-[var(--text-3)] font-bold truncate leading-tight uppercase tracking-tight">
                      {poem.poet} · {poem.verses.length} بيت
                    </p>
                  </div>

                  {/* Actions */}
                  <div className="flex items-center gap-2 shrink-0 translate-x-[10px] translate-y-[6px]">
                    <Button
                      variant="primary"
                      size="sm"
                      onClick={() => onStudy(poem)}
                      className="h-9 !px-[14px]"
                    >
                      <Play
                        size={14}
                        strokeWidth={3}
                ...
                {poem.verses[0] && (
                  <div className="p-4 rounded-2xl bg-[var(--bg-2)] border border-[var(--border-0)]">
                    ...
                    >
                      ...
                      {poem.verses[0].ajar && (
                        <span className="text-[var(--text-3)] mx-3">
                          ◇
                        </span>
                      )}
                ...
                <div className="flex items-center justify-between gap-6 px-1">
                  <div className="flex-1 flex items-center gap-3 min-w-0">
                    ...
                    <span className="text-sm font-extrabold text-[var(--text-2)] tabular-nums">
                      {learnedPct}%
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {progress && progress.mastered > 0 && (
                      <Badge variant="success">
                        {progress.mastered} متقن
                      </Badge>
                    )}

                    <span className="text-[11px] text-[var(--text-3)] font-bold opacity-60 tabular-nums relative !right-[-16px]">
                      البيت {currentVerse}
                    </span>
                  </div>
                </div>
                ...
                {/* Dropdown menu */}
                {menuOpen === poem.id && (
                  <div className="mt-3 pt-3 border-t border-[var(--border-0)] flex items-center gap-1 animate-fade-in relative !right-[10px]">
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => {
