...
export default function StatsView({ poem, stats, errorLog, onClose }: Props) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
      className="w-full max-w-lg !h-[50vh] !max-h-[50vh] !min-h-0 glass-modal rounded-xl overflow-hidden animate-scale-in flex flex-col relative -top-[20px]"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-0)] shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-[var(--text-0)] relative right-[8px]">
              إحصائيات
            </h2>

            <p className="text-[11px] text-[var(--text-3)] relative right-[10px]">
        ...
        <div className="p-6 !min-h-0 !flex-1 !overflow-y-auto flex flex-col gap-6">
          <div className="relative left-[48px] right-[8px] min-h-0 pb-[200px]">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                >
                  <p
                    className={`text-xl font-extrabold ${s.color} tabular-nums relative translate-x-[15px]`}
                  >
                    {s.value}
                  </p>

                  <p className="text-[10px] text-[var(--text-3)] font-bold uppercase tracking-wider relative translate-x-[15px] top-[-3px]">
                    {s.label}
                  </p>
                </div>
              ))}
            ...
            <div className="flex gap-4 justify-center items-center py-2 translate-y-[6px]">
              {[
                {
                  icon: Flame,
                  value: streak,
                ...
                <div key={i} className="flex flex-col items-center gap-1.5">
                  ...
                  <div className="flex flex-col items-center">
                    <p className="text-sm font-extrabold text-[var(--text-0)] leading-none truncate max-w-[80px]">
                      {item.value}
                    </p>

                    <p className="text-[9px] text-[var(--text-3)] font-bold uppercase tracking-tight mt-1 opacity-60">
                      {item.label}
                    </p>
                  </div>
                </div>
            ...
            {/* مستويات الحفظ */}
            <div className="flex flex-col gap-3">
              <h4 className="text-xs font-bold text-[var(--text-1)] px-1 uppercase tracking-widest flex items-center gap-1 translate-y-[8px] relative right-[10px]">
              ...
              <div className="flex flex-col gap-2 glass-inner rounded-3xl p-5 border border-[var(--border-0)]">
                {[
                  ...
                  >
                    <span
                      className={`text-[10px] font-bold w-10 shrink-0 relative right-[16px] ${x.cl}`}
                    >
                      {x.l}
                    </span>

                    <div className="w-[200px] h-2.5 rounded-full bg-black/10 dark:bg-white/10 overflow-hidden relative border border-black/5 shadow-inner">
                      <div
                        className={`absolute top-0 bottom-0 right-0 rounded-full transition-all duration-1000 ease-out ${x.color}`}
                        style={{
                          width:
                            total > 0
                              ? `${Math.round((x.c / total) * 100)}%`
                              : '0%'
                        }}
                      />
                    </div>

                    <span className="text-[11px] font-extrabold text-[var(--text-2)] w-8 text-left tabular-nums translate-x-[24px]">
                      {x.c}
                    </span>
                  </div>
                ))}
            ...
            {weakIndices.length > 0 && (
              <div>
               <h4 className="text-xs font-medium text-[var(--text-1)] mb-2 flex items-center gap-0.5 -translate-x-[10px]">
                  <AlertTriangle
                    size={13}
                    strokeWidth={1.5}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-[10px] text-[var(--text-3)]">
                            {getStabilityLabel(vs.memory.standalone)}
                          </span>
                        </div>
                      </div>
                    );
            ...
            {poemErrors.length > 0 && (
              <div>
                <h4 className="text-xs font-medium text-[var(--text-1)] mb-2">
                  آخر الأخطاء
                </h4>
            ...
            {/* مساحة فارغة 80px بعد نهاية آخر بطاقة */}
